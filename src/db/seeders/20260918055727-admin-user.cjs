...
module.exports = {
  async up(queryInterface, Sequelize) {
    if (process.env.NODE_ENV === 'production') {
      console.log('Skipping administrador user seed in production environment.');
      return;
    }
    ...
    if (!roleId || !documentTypeId || !genderId || !academicLevelId || !municipalityId) {
      throw new Error(
        'Cannot seed administrador user: Required lookup data (role, document type, gender, academic level, or municipality) was not found.'
      );
    }
