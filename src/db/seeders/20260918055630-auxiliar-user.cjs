...
module.exports = {
  async up(queryInterface, Sequelize) {
    if (process.env.NODE_ENV === 'production') {
      console.log('Skipping auxiliar user seed in production environment.');
      return;
    }

    // 1. Safe FK lookups
    const [roles] = await queryInterface.sequelize.query(
      `SELECT id_rol FROM rol WHERE nombre_rol = 'Auxiliar' LIMIT 1;`
    );
    ...
    if (!roleId || !documentTypeId || !genderId || !academicLevelId || !municipalityId) {
      throw new Error(
        'Cannot seed auxiliar user: Required lookup data (role, document type, gender, academic level, or municipality) was not found.'
      );
    }
